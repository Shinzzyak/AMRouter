                ...
                if is_cf_email:
                    ...
                    for pat in patterns:
                        ...
                        if links:
                            ...
                            return link
                    # Fallback: extract 6-digit OTP token (Cloudflare login token)
                    otp_match = re.search(r'\b(\d{6})\b', body)
                    if otp_match:
                        otp_code = otp_match.group(1)
                        log_step(f"OTP token ditemukan: {otp_code}")
...
def main():
    ...
    parser.add_argument("--clean", action="store_true", help="Ignored route arg")
    parser.add_argument("--invite-link", default="", help="Ignored route arg")
    parser.add_argument("--signup-method", default="", help="Ignored route arg")
    ...
    if proxy_dict:
        ...
        # geoip is best-effort: only enable if the extra is actually installed.
        try:
            import importlib.util
            if importlib.util.find_spec("camoufox_geoip") is not None or os.environ.get("CAMOUFOX_GEOIP"):
                launch_kwargs["geoip"] = True  # match geolocation to proxy IP (suppresses LeakWarning)
            else:
                log_step("geoip extra tidak terinstall — lanjut tanpa geoip (proxy tetap jalan)")
        except Exception:
            pass
    ...
    with browser_ctx as browser:
        ...
        # page.set_viewport_size({"width": 1920, "height": 1080}) # disabled for camoufox compatibility
        # Swallow pageerror events to prevent Playwright internal crash
        # (pageError.location can be undefined in certain Cloudflare JS contexts)
        ...
        try:
            ...
            err = str(e1).lower()
            if "proxy" in err or "sec_error" in err or "net::" in err or "ns_error" in err or "bad_gateway" in err:
                log_step(f"Proxy bapuk: {e1}")
                print('{"step": "PROXY_FAILED", "error": "Bad proxy connection"}', flush=True)
                sys.exit(3)
            elif "interrupted" in err:
                log_step("Navigation interrupted by redirect, waiting for page to settle...")
                time.sleep(5)
                try:
                ...
                log_step(f"Page load timeout: {e1}")
                print('{"step": "PROXY_FAILED", "error": "Timeout loading page"}', flush=True)
                sys.exit(3)
            else:
                raise
        ...
        form_found = False
        # Patient poll: do NOT reload (reload re-triggers Turnstile from scratch).
        # Just wait for the email input to appear; Turnstile resolves on its own.
        deadline = time.time() + 90.0
        while time.time() < deadline:
            try:
                if page.locator("input[name='email'], input[autocomplete='email'], input[type='email']").first.is_visible(timeout=3000):
                    form_found = True
                    break
            except Exception:
                pass
            # If still on Turnstile, nudge the checkbox occasionally
            if is_on_turnstile_page(page):
                try_click_turnstile_checkbox(page)
            time.sleep(3.0)
        if not form_found:
            page.screenshot(path="/root/.openclaw/workspace/AMRouter/backend/cf_form_error.png")
            die("Form signup tidak muncul (tersimpan screenshot cf_form_error.png)")
        ...
        email_filled = False
        # Retry email fill up to 3 times with wait (page may re-render after patient poll)
        for _email_attempt in range(3):
            # First try: JS fill (bypasses Turnstile overlay blocking)
            try:
                _js_result = page.evaluate(f"""() => {{
                ...
                if _js_result:
                    email_filled = True
                    log_step(f"Email filled via JS (attempt {_email_attempt+1})")
                    break
            except Exception as e:
                log_step(f"JS fill error: {e}")
            ...
            for sel in email_sel:
                try:
                    ...
                    if el.is_visible(timeout=5000):
                        ...
                        el.fill(args.email)
                        email_filled = True
                        log_step(f"Email filled via: {sel} (attempt {_email_attempt+1})")
                        break
                except Exception:
                    continue
            if email_filled:
                break
            log_step(f"Email fill attempt {_email_attempt+1} failed, waiting 3s...")
            time.sleep(3)
        if not email_filled:
            try:
                page.screenshot(path="/tmp/cf_email_fail.png")
                _all_inputs = page.evaluate("() => Array.from(document.querySelectorAll('input')).map(i => ({type:i.type,name:i.name,id:i.id,visible:i.offsetParent!==null}))")
                log_step(f"DEBUG inputs: {_all_inputs}")
            except Exception:
                pass
        ...
        submitted = False
        # First try: JS click (bypasses Turnstile overlay blocking)
        try:
            _js_submitted = page.evaluate("""() => {
                const btns = document.querySelectorAll('button');
                for (const b of btns) {
                    const txt = (b.innerText || '').toLowerCase();
                    if (txt.includes('sign up') || txt.includes('create account') || txt.includes('get started')) {
                        b.click();
                        return true;
                    }
                }
            ...
            if _js_submitted:
                submitted = True
                log_step("Submit clicked via JS")
        except Exception as e:
            log_step(f"JS submit error: {e}")
            ...
            for sel in submit_selectors:
                try:
                    ...
                    if btn.is_visible(timeout=2000):
                        ...
                        submitted = True
                        log_step(f"Submit clicked via: {sel}")
                        break
                except Exception:
                    continue
        if not submitted:
        ...
        if ammail_ok and not email_already_registered:
            ...
            if verify_link:
                if verify_link.startswith("OTP:"):
                    # OTP token — enter code on the challenge page
                    otp_code = verify_link.split(":", 1)[1]
                    log_step(f"Memasukkan OTP token: {otp_code}")
                    try:
                        # Find OTP input field (various selectors)
                        otp_selectors = [
                            "input[name='token']",
                            "input[name='code']",
                            "input[name='otp']",
                        ...
                        for sel in otp_selectors:
                            try:
                                ...
                                if el.is_visible(timeout=2000):
                                    el.fill(otp_code)
                                    otp_filled = True
                                    log_step(f"OTP filled via selector: {sel}")
                                    break
                            except Exception:
                                continue
                        if not otp_filled:
                            log_step("Warning: OTP input field not found")
                        ...
                        for sel in submit_selectors:
                            try:
                                ...
                                if btn.is_visible(timeout=1000):
                                    btn.click()
                                    log_step("OTP submit clicked")
                                    break
                            except Exception:
                                continue
                        time.sleep(5)
                    except Exception as e:
                        log_step(f"Warning OTP input: {e}")
                else:
        ...
        if _m_verify:
            ...
            try:
                ...
                if _m_redir:
                    ...
                    # Wait for login form — Cloudflare login uses various selectors
                    login_selectors = [
                        "input[name='email']",
                        "input[autocomplete='email']",
                        "input[type='email']",
                    ...
                    login_found = False
                    for _lsel in login_selectors:
                        try:
                            page.wait_for_selector(_lsel, timeout=5000)
                            login_found = True
                            log_step(f"Login form ditemukan: {_lsel}")
                            break
                        except Exception:
                            continue
                    if not login_found:
                        # Wait extra for Cloudflare SPA to render
                        time.sleep(5)
                        for _lsel in login_selectors:
                            try:
                                if page.locator(_lsel).first.is_visible(timeout=3000):
                                    login_found = True
                                    log_step(f"Login form ditemukan (retry): {_lsel}")
                                    break
                            except Exception:
                                continue
                    if not login_found:
                    ...
                    if not _early_account_id:
                        ...
                        # CF Login Step 1: Fill email (JS bypass for Turnstile overlay)
                        email_filled = False
                        try:
                            _js_email = page.evaluate(f"""() => {{
                                const el = document.querySelector('input[name="email"]') ||
                                           document.querySelector('input[type="email"]') ||
                                           document.querySelector('input[autocomplete="email"]') ||
                            ...
                            if _js_email:
                                email_filled = True
                                log_step("Login email filled via JS")
                        except Exception as e:
                            log_step(f"JS login email error: {e}")
                        # Fallback: Playwright fill
                        if not email_filled:
                            for sel in [
                                "input[name='email']",
                        ...
                        pw_filled = False
                        try:
                            _js_pw = page.evaluate(f"""() => {{
                                const el = document.querySelector('input[type="password"]') ||
                                           document.querySelector('input[name="password"]');
                                if (!el) return false;
                                el.focus();
                                el.value = '{args.password}';
                                el.dispatchEvent(new Event('input', {{bubbles: true}}));
                                el.dispatchEvent(new Event('change', {{bubbles: true}}));
                                return true;
                            }}""")
                            if _js_pw:
                                pw_filled = True
                                log_step("Login password filled via JS")
                        except Exception as e:
                            log_step(f"JS login pw error: {e}")
                        # Fallback: Playwright fill
                        if not pw_filled:
                            for pw_sel in [
                                "input[type='password']",
                                ...
                                try:
                                    ...
                                    log_step(f"Login pw try {pw_sel}: {type(ex).__name__}")
                                    continue
        ...
        def create_token_via_session(page):
            """Use page.request.fetch() to create API token via CF's REST API.
            Carries browser session cookies (CF_Authorization). No OTP/Global API Key needed.
            """
            ...
            try:
                ...
                # Step 0: Extract CSRF token from CF session cookie
                csrf_token = None
                try:
                    csrf_token = page.evaluate("""
                        () => {
                            // CF stores CSRF in a meta tag or cookie
                            const meta = document.querySelector('meta[name="csrf-token"]');
                            if (meta) return meta.content;
                            // Also check for X-CF-Token header in service worker
                            const cookies = document.cookie.split(';').map(c => c.trim());
                            for (const c of cookies) {
                                if (c.startsWith('_csrf_token=')) return c.split('=')[1];
                            }
                            return null;
                        }
                    """)
                    if csrf_token:
                        log_step(f"CSRF token found: {csrf_token[:12]}...")
                        common_headers["X-CSRF-Token"] = csrf_token
                except Exception as cse:
                    log_step(f"CSRF extraction: {cse}")

                # Step 0b: Also try to get x-csrf-token from a HEAD/OPTIONS preflight
                # (CF dashboard sometimes sets it via Set-Cookie)
                if not csrf_token:
                    try:
                        preflight = page.request.fetch(f"{base}/accounts?per_page=1", method="OPTIONS", headers=common_headers)
                        # The response headers might set CSRF
                        log_step(f"Preflight status: {preflight.status}")
                    except Exception:
                        pass

                # Step 1: Verify session works — list accounts
                accounts_resp = page.request.fetch(
                    f"{base}/accounts?per_page=5",
                    method="GET",
                    headers=common_headers,
                )
                log_step(f"Accounts list status: {accounts_resp.status}")
                if not accounts_resp.ok:
                    log_step(f"Accounts list failed: HTTP {accounts_resp.status} — {accounts_resp.status_text}")
                    # Try with page.evaluate fetch (carries cookies automatically)
                    try:
                        ...
                        """)
                        log_step(f"Accounts via evaluate: {str(eval_result)[:200]}")
                        if isinstance(eval_result, dict) and eval_result.get('success'):
                            accounts_data = eval_result
                        else:
                            log_step("Session auth failed — cannot create token without login")
                            return None
                    except Exception as ee:
                        log_step(f"Evaluate fallback failed: {ee}")
                        return None
                else:
                    accounts_data = accounts_resp.json()

                # Get account_id from accounts list if not provided
                nonlocal account_id
                if not account_id and accounts_data.get('success'):
                    accs = accounts_data.get('result', [])
                    if accs:
                        account_id = accs[0].get('id', '')
                        log_step(f"Account ID from session: {account_id[:8]}...")
                if not account_id:
                    log_step("No account_id available — cannot create token")
                    return None

                # Step 2: Get Workers AI permission group id
                pg_resp = page.request.fetch(
                    f"{base}/user/tokens/permission_groups",
                    method="GET",
                    headers=common_headers,
                )
                log_step(f"Permission groups status: {pg_resp.status}")
                if not pg_resp.ok:
                    # Try via page.evaluate
                    try:
                        pg_eval = page.evaluate("""
                            async () => {
                                const r = await fetch('https://api.cloudflare.com/client/v4/user/tokens/permission_groups', {
                                    credentials: 'include',
                                    headers: {'Accept': 'application/json'}
                                });
                                return await r.json();
                        ...
                        """)
                        log_step(f"Permission groups via evaluate: {str(pg_eval)[:200]}")
                        if isinstance(pg_eval, dict) and pg_eval.get('success'):
                            pg_data = pg_eval
                        else:
                            log_step(f"Permission groups failed")
                            return None
                    except Exception as pge:
                        log_step(f"Permission groups evaluate failed: {pge}")
                        return None
                else:
                ...
                # Step 3: Create scoped API token
                payload = {
                    "name": "9router-workers-ai",
                    "policies": [{
                        "effect": "allow",
                ...
                if tok_data.get("success"):
                    token_val = tok_data["result"].get("value")
                    log_step(f"SUCCESS: Token created via session ({token_val[:12]}...)")
                    return token_val
                err = tok_data.get("errors", [{}])[0].get("message", "unknown")
                log_step(f"Token create failed: {err}")
                # Fallback: try via page.evaluate
                try:
                    eval_token = page.evaluate(f"""
                        async () => {{
                    ...
                    """)
                    log_step(f"Token via evaluate: {str(eval_token)[:200]}")
                    if isinstance(eval_token, dict) and eval_token.get('success'):
                        token_val = eval_token['result'].get('value')
                        log_step(f"SUCCESS: Token via evaluate ({token_val[:12]}...)")
                        return token_val
                except Exception as ete:
                    log_step(f"Evaluate token create failed: {ete}")
            except Exception as e:
